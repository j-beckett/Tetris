...
{	
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(guiEvent))
		{
			if (guiEvent.type == sf::Event::Closed)	// handle close button clicked
			{
				window.close();
			}
			else if (guiEvent.type == sf::Event::KeyPressed) // handle key press
			{
				game.onKeyPressed(guiEvent);	
			}
		}
